...
const TopPickProfileScreen = ({ route, navigation }) => {
  ...
  // Simplified Like Handler (Always Requesting)
  const handleLike = async () => {
    try {
      ...
      // Always record LIKE interaction
      const response = await recordInteraction(user._id || user.id, 'LIKE');

      // Wait for heart animation to play out (matching MainScreen feel ~3.2s)
      ...
      if (response?.match?.isMutual) {
        ...
        // Just go back after like success (Request Sent)
        navigation.goBack();
      }
    } catch (error) {
      console.error('Like error:', error);
      setLoading(false);
      // Even if error, we probably want to let the user know, but maybe after animation?
      // For now, prompt immediately
      showAlert('Error', 'Failed to like. Please try again.', 'error');
    }
  };

  const handleSendSuperLike = async (profile, comment) => {
    try {
      setLoading(true);
      ...
      const response = await recordInteraction(user._id || user.id, 'SUPERLIKE');
      
      // Wait for heart animation to play out
      await new Promise(resolve => setTimeout(resolve, 3200));
      ...
      if (response?.match?.isMutual) {
        showAlert(
          "It's a Match!",
          `You and ${user.displayName || user.name} liked each other!`,
          'success',
          null,
          [
             {
        ...
        // Just go back after super like success
        navigation.goBack();
      }
    } catch (error) {
  ...
  const handlePass = async () => {
    try {
      ...
      // Always record PASS interaction
      const apiPromise = recordInteraction(user._id || user.id, 'PASS');
      ...
      showAlert('Error', 'Failed to pass. Please try again.', 'error');
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Action Buttons */}
        <View style={styles.actionsContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => setShowSuperLikeModal(true)}
            disabled={loading}
          >
            {loading && actionType === 'SUPERLIKE' ? (
              <View style={[styles.actionButtonCircle, styles.superLikeCircle]}>
                <Animated.View
                  style={{
                    transform: [
                      { scale: heartScale }, // Reusing heartScale for pulse
                      {
                        rotate: heartRotate.interpolate({ // Reusing heartRotate for spin
                          inputRange: [0, 1],
              ...
              <View style={[styles.actionButtonCircle, styles.superLikeCircle, loading && styles.disabledButton]}>
                <Ionicons name="star" size={28} color="#D4AF37" />
              </View>
            )}
          </TouchableOpacity>
      ...
      <HeartOverlay
        ...
        particleOpacity={likeParticleOpacity}
        particles={particles}
      />
...
export default TopPickProfileScreen;
